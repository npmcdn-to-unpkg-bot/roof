<div class="container offset_vertical_60">
	...
	<form action="" class="offset_vertical_15 jus">
		<select style="width: 170px;" name="country" class="input_select input jus__item">
			<option value="">СТРАНА</option>
			@foreach ($countries as $country)
				<option value="{{$country->id}}" {{Request::get('country')==$country->id?'selected':''}}>{{$country->name}}</option>
			@endforeach
		</select>
		<select style="width: 170px;" name="city" class="input_select input jus__item">
			<option value="">ГОРОД</option>
			@foreach ($cities as $city)
				<option value="{{$city->id}}" {{Request::get('city')==$city->id?'selected':''}}>{{$city->name}}</option>
			@endforeach
		</select>
		<select style="width: 190px;" name="type" class="input_select input jus__item">
			<option value="">ТИП ОБЪЕКТОВ</option>
			@foreach ($types as $type)
				<option value="{{$type}}" {{Request::get('type')==$type?'selected':''}}>{{$type}}</option>
			@endforeach
		</select>
		<select style="width: 280px;" name="speciality" class="input_select input jus__item">
		...
		<select style="width: 170px;" name="seasonality" class="input_select input jus__item">
