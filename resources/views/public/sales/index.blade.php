		...
		<div class="container__row">
			<div class="container__col-8">
				...
				<div class="market-news offset_bottom_30">
					@if ($sale->image) <a href="{{route('sales.show', $sale)}}"><img src="/imagecache/240x200/{{$sale->image}}" alt="" class="market-news__image"></a> @endif
					<a href="{{route('sales.show', $sale)}}" class="market-news__title">{{$sale->title}}</a>
					<div class="market-news__text">{{$sale->entry}}</div>
				</div>
				@endforeach

				@include('public.pagenav',['items'=>$sales])
